...
async function getProduct(slug: string): Promise<Product | null> {
  ...
  const { data } = await supabase
    .from("products")
    .select("*, images:product_images(*), variants:product_variants(*), addons:product_addons(*), tags:product_tags(tag:tags(*))")
    .eq("slug", slug)
    .eq("status", "published")
    .single();

  if (!data) return null;
  type RawProduct = Omit<Product, "tags"> & { tags: { tag: NonNullable<Product["tags"]>[number] }[] };
  const raw = data as unknown as RawProduct;
  return { ...raw, tags: raw.tags?.map((t) => t.tag) ?? [] };
}
...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="section py-12">
      <ViewTracker event="product_view" params={{ product: product.slug }} />
      <JsonLd
        data={{
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        <div className="arch-frame relative aspect-[4/5] bg-blush-light">
          {images[0] ? (
            <Image src={images[0].url} alt={images[0].alt || product.name} fill sizes="50vw" className="object-cover" />
          ) : (
        ...
        <div>
          ...
          {product.weight_or_size && <p className="mt-1 text-sm text-ink-soft">{product.weight_or_size}</p>}
          {product.yield_info && <p className="text-sm text-ink-soft">{product.yield_info}</p>}
          ...
          {product.full_description && (
            <p className="mt-4 max-w-[55ch] text-ink-soft leading-relaxed">{product.full_description}</p>
          )}

          {product.tags && product.tags.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-2">
              {product.tags.map((t) => (
                <span key={t.id} className="rounded-full bg-pine/10 px-3 py-1 text-xs text-pine">
                  {t.name}
                </span>
              ))}
            </div>
