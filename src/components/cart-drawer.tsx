...
export function CartDrawer() {
  const { storeId, lines, drawerOpen, closeDrawer, removeLine, updateQuantity, clear } = useCartStore();
    ...
    <div className="fixed inset-0 z-[60] flex justify-end">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {lines.length === 0 ? (
            <p className="mt-10 text-center text-sm text-ink-soft">Seu pedido está vazio.</p>
          ) : (
            <ul className="space-y-4">
              {lines.map((line, index) => (
                <li key={index} className="flex gap-3 border-b border-ink/5 pb-4">
                  <div className="flex-1">
                    <p className="font-medium text-ink">{line.productName}</p>
                    {line.variant && <p className="text-xs text-ink-soft">Variação: {line.variant}</p>}
                    {line.addons && line.addons.length > 0 && (
                      <p className="text-xs text-ink-soft">Adicionais: {line.addons.join(", ")}</p>
                    )}
                    {line.note && <p className="text-xs text-ink-soft">Obs.: {line.note}</p>}
                  ...
                  <div className="flex flex-col items-end justify-between">
                    <p className="text-sm font-medium">{formatBRL(line.unitPrice * line.quantity)}</p>
                    <button
                      type="button"
        ...
        {lines.length > 0 && (
          <div className="border-t border-ink/10 px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              />
              <select value={mode} onChange={(e) => setMode(e.target.value as "Retirada" | "Delivery")} className="input">
            ...
            <a
              href={canCheckout ? buildWhatsAppOrderUrl({ store: store!, lines, customerName: name, mode, desiredTime }) : undefined}
              target="_blank"
              rel="noreferrer"
              ...
              onClick={(e) => {
                ...
                track("checkout_started", { store: store?.slug, items: lines.length, value: subtotal });
              }}
              className={
                "block rounded-full px-6 py-3.5 text-center text-sm font-medium " +
                (canCheckout ? "bg-pine text-cream-soft hover:bg-pine-dark" : "bg-ink/10 text-ink-soft/50")
              }
            >
